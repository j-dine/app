...
export async function addTransactionToBudget(
    ...
    try {
        ...
        const totalTransactions = budget.transactions.reduce((sum: any, transaction: { amount: any; }) => {
...
export async function getTransactionsByEmailAndPeriod(email: string, period: string) {
    try {
        ...
        const transactions = user.budgets.flatMap((budjet: { transactions: any[]; name: any; id: any; }) =>
...
export async function getTotalTransactionAmount(email: string) {
    try {
        ...
        const totalAmount = user.budgets.reduce((sum: any, budgets: { transactions: any[]; }) => {
...
export async function getTotalTransactionCount(email: string) {
    try {
        ...
        const totalCount = user.budgets.reduce((count: any, budget: { transactions: string | any[]; }) => {
...
export async function getReachedBudgets(email: string) {
    try {
        ...
        const reachedBudgets = user.budgets.filter((budjet: { transactions: any[]; amount: number; }) => {
...
export async function getUserBudgetData(email: string) {
    try {
        ...
        const data = user.budgets.map((budget: { transactions: any[]; name: any; amount: any; }) => {
...
export const getLastTransactions = async (email: string) => {
    try {
        ...
        const transactionsWithBudgetName = transactions.map((transaction: { budget: { name: string } | null }) => ({
            ...transaction,
